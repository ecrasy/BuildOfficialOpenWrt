...
# Modified Time: 2022-10-25 00:58:18 UTC
#########################################################################
...
cp etc/banner package/base-files/files/etc/banner
cd -
echo "Replace default ssh banner"

# Adding model.sh to remove annoying board name for Intel J4125
cd $GITHUB_WORKSPACE/data
cp etc/model.sh package/base-files/files/etc/
cd -
...
cd $GITHUB_WORKSPACE/data
cp etc/99-ipv6 package/base-files/files/etc/hotplug.d/iface/99-ipv6
cd -
